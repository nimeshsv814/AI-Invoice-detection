...
export async function checkForDuplicates(input: DuplicateCheckInput): Promise<DuplicateCheckResult> {
  ...
  const candidates = await query<any>(
    `SELECT id, invoice_number, vendor_name, total_amount, invoice_date, po_number
     FROM invoices
     WHERE id != $1
  ...
  let bestMatch: { id: string; score: number; fields: string[]; fieldScores: Record<string, number> } | null = null;
...
function computeSimilarity(
  ...
  candidate: any
): { score: number; fields: string[]; fieldScores: Record<string, number> } {
  const fields: string[] = [];
  ...
  if (input.invoiceNumber && candidate.invoice_number) {
    ...
    if (s > 0) { fields.push('invoice_number'); fieldScores.invoiceNumber = s; }
  ...
  if (input.vendorName && candidate.vendor_name) {
    ...
    if (s > 60) { fields.push('vendor_name'); fieldScores.vendorName = s; }
  ...
  if (input.totalAmount != null && candidate.total_amount != null) {
    ...
    if (s > 0) { fields.push('total_amount'); fieldScores.totalAmount = s; }
  ...
  if (input.invoiceDate && candidate.invoice_date) {
    ...
    if (s > 0) { fields.push('invoice_date'); fieldScores.invoiceDate = s; }
  ...
  if (input.poNumber && candidate.po_number) {
    ...
    if (s > 0) { fields.push('po_number'); fieldScores.poNumber = s; }
...
export async function getDuplicateAlerts(): Promise<any[]> {
...
export async function resolveDuplicate(
  id: string, status: 'confirmed_duplicate' | 'false_positive', resolvedBy: string, notes?: string
): Promise<void> {
  await query(
    `UPDATE duplicate_detection_results
     SET resolution_status = $2, resolved_by = $3, resolved_at = NOW(), notes = $4
     WHERE id = $1`,
    [id, status, resolvedBy, notes || null]
  );
}
