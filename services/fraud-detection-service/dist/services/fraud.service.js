...
exports.getFraudScore = getFraudScore;
...
// ─────────────────────────────────────────────────────────────────────────────
async function analyzeInvoice(invoiceId, ocrData) {
    const indicators = [];
    ...
    await (0, database_1.query)(`INSERT INTO fraud_scores (invoice_id, risk_score, risk_level, fraud_indicators, anomaly_details, explanation, recommendations)
     ...
     ON CONFLICT (invoice_id) DO UPDATE SET
       ...
       analyzed_at = NOW()`, [invoiceId, riskScore, riskLevel, JSON.stringify(indicators),
...
async function getVendorHistory(vendorName) {
    if (!vendorName)
        return null;
    const result = await (0, database_1.query)(`SELECT COUNT(*) AS total, AVG(total_amount) AS avg_amount
     FROM invoices WHERE LOWER(vendor_name) = LOWER($1) AND status = 'approved'`, [vendorName]);
    const row = result.rows[0];
    return { totalInvoices: parseInt(row.total, 10), avgAmount: parseFloat(row.avg_amount) || 0 };
}
async function getRecentInvoiceCount(vendorName, days) {
    if (!vendorName)
        return 0;
    const result = await (0, database_1.query)(`SELECT COUNT(*) FROM invoices
     WHERE LOWER(vendor_name) = LOWER($1) AND created_at >= NOW() - INTERVAL '${days} days'`, [vendorName]);
    return parseInt(result.rows[0].count, 10);
}
function generateExplanation(score, level, indicators, details) {
    if (indicators.length === 0)
        return 'No fraud indicators detected. Invoice appears legitimate.';
    const readable = {
...
function generateRecommendations(level, indicators) {
    const recs = [];
    if (level === 'critical') {
    ...
    if (indicators.includes('missing_po_number'))
        recs.push('Request PO number from vendor');
    if (indicators.includes('unusual_amount'))
        recs.push('Verify scope of work justifies invoice amount');
    if (indicators.includes('new_vendor'))
        recs.push('Complete vendor onboarding verification');
    return recs;
}
async function getFraudScore(invoiceId) {
    const result = await (0, database_1.query)(`SELECT * FROM fraud_scores WHERE invoice_id = $1 ORDER BY analyzed_at DESC LIMIT 1`, [invoiceId]);
    return result.rows[0] || null;
}
async function getFraudTrends() {
...
async function getHighRiskVendors() {
    const result = await (0, database_1.query)(`
    SELECT
      i.vendor_name,
