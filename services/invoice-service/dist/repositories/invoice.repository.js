...
async function createInvoice(invoice) {
    const result = await (0, database_1.query)(`INSERT INTO invoices (file_name, file_path, file_size, file_type, status, uploaded_by)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING *`, [
        invoice.fileName,
        invoice.filePath,
...
async function createLineItem(item) {
    const result = await (0, database_1.query)(`INSERT INTO invoice_line_items 
...
async function findInvoiceById(id) {
    const invoiceResult = await (0, database_1.query)(`SELECT i.*, u.first_name || ' ' || u.last_name as uploader_name
     FROM invoices i
    ...
    const lineItemsResult = await (0, database_1.query)(`SELECT * FROM invoice_line_items WHERE invoice_id = $1 ORDER BY line_number ASC`, [id]);
    invoice.lineItems = lineItemsResult.rows;
    // Fetch related OCR, duplicate, and fraud records if they exist
    const ocrResult = await (0, database_1.query)(`SELECT * FROM ocr_results WHERE invoice_id = $1 LIMIT 1`, [id]);
    const fraudResult = await (0, database_1.query)(`SELECT * FROM fraud_scores WHERE invoice_id = $1 LIMIT 1`, [id]);
    const duplicateResult = await (0, database_1.query)(`SELECT ddr.*, ci.invoice_number AS compared_invoice_number
     FROM duplicate_detection_results ddr
     LEFT JOIN invoices ci ON ddr.compared_invoice_id = ci.id
     WHERE ddr.invoice_id = $1 LIMIT 1`, [id]);
...
async function updateInvoice(id, updates) {
    ...
    await (0, database_1.query)(`UPDATE invoices SET ${setClauses.join(', ')}, updated_at = NOW() WHERE id = $1`, [id, ...values]);
    return findInvoiceById(id);
}
async function findInvoices(filters) {
  ...
  `;
    ...
    const invoicesResult = await (0, database_1.query)(selectQuery, params);
    return {
...
async function getDashboardStats() {
    const totalInvoicesResult = await (0, database_1.query)(`SELECT COUNT(*) FROM invoices`);
    const statusCountsResult = await (0, database_1.query)(`SELECT status, COUNT(*), SUM(total_amount) as total_val FROM invoices GROUP BY status`);
    const pendingApprovalsResult = await (0, database_1.query)(`SELECT COUNT(*) FROM invoices WHERE status IN ('pending_review', 'ocr_complete', 'duplicate_check', 'fraud_check')`);
    const totalSpendResult = await (0, database_1.query)(`SELECT SUM(total_amount) FROM invoices WHERE status = 'approved'`);
    const recentInvoicesResult = await (0, database_1.query)(`
...
async function saveLineItems(invoiceId, items) {
    await (0, database_1.query)(`DELETE FROM invoice_line_items WHERE invoice_id = $1`, [invoiceId]);
